...
const PILL_TEXTURE = {
  backgroundColor: "var(--color-nx-surface)",
  backgroundImage:
...
function StatPill({ icon, value, alt, onClick }: { icon: string; value: string; alt: string; onClick: () => void }) {
  return (
    <div
      onClick={onClick}
      className={`flex h-14 w-40 cursor-pointer items-center gap-2 rounded-lg border-2 border-[color:var(--color-nx-gold)]/70 ps-2 pe-5 py-1 ${PILL_SHADOW}`}
      style={PILL_TEXTURE}
    >
...
function IconTile({ src, alt, size = "w-18 h-18", onClick }: { src: string; alt: string; size?: string; onClick: () => void }) {
  return (
...
export function GamingHeader() {
  ...
  return (
    <header className="sticky top-0 z-40 w-full overflow-hidden border-y-2 border-[color:var(--color-nx-gold)]/60 bg-gradient-to-r from-[color:var(--color-nx-bg)] via-[color:var(--color-nx-surface-2)] to-[color:var(--color-nx-bg)] px-24 shadow-lg">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_35%_50%,rgba(147,51,234,0.35),transparent_65%)]" />
      ...
      <div className="homepages-scrollbar-thin relative flex items-center gap-6 overflow-x-auto px-4">
        ...
        <div
          ...
          className="relative h-16 w-16 shrink-0 cursor-pointer overflow-hidden rounded border border-[color:var(--color-nx-gold)]/70 bg-gradient-to-b from-[color:var(--color-nx-violet)] via-[color:var(--color-nx-purple)] to-[color:var(--color-nx-bg)] shadow-lg"
        >
          {user ? (
        ...
        <div className="ms-3 flex shrink-0 items-center gap-10">
          <StatPill icon={HOME_IMAGES.amount} alt="Amount" value={user ? user.balance.toFixed(2) : "45,230"} onClick={requireAuth()} />
        ...
        <div className="absolute left-1/2 ms-3 flex -translate-x-1/2 items-center gap-8">
          <button
            type="button"
            onClick={requireAuth()}
            className="relative cursor-pointer overflow-hidden rounded-2xl border-2 border-[color:var(--color-nx-gold)] bg-gradient-to-b from-[color:var(--color-nx-gold)] to-amber-500 px-12 py-3 font-title text-2xl font-extrabold text-slate-900 shadow-[0_4px_10px_rgba(0,0,0,0.45),inset_0_2px_3px_rgba(255,255,255,0.35),inset_0_-3px_6px_rgba(0,0,0,0.35)] transition-transform active:scale-95"
          >
          ...
          <button
            type="button"
            onClick={requireAuth()}
            className="relative cursor-pointer overflow-hidden rounded-2xl border-2 border-[color:var(--color-nx-cyan)] bg-gradient-to-b from-[color:var(--color-nx-violet)] to-[color:var(--color-nx-purple)] px-12 py-3 font-title text-2xl font-extrabold text-white shadow-[0_4px_10px_rgba(0,0,0,0.45),inset_0_2px_3px_rgba(255,255,255,0.35),inset_0_-3px_6px_rgba(0,0,0,0.35)] transition-transform active:scale-95"
          >
            <span className="pointer-events-none absolute inset-x-0 top-0 h-1/2 bg-gradient-to-b from-white/40 to-transparent" />
